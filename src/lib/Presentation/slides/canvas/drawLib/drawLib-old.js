...
export default class DrawLib {
 ...
 //  rect     
 rect(x, y, width, height, color = 'white', filled = true,dash = 0, gap = 0) {
        if (filled) {
            this.ctx.fillStyle = color;
...
// circle  
  circle(x, y, radius, color = 'black', fill = false,startingAngle=0,endingAngle=Math.PI * 2,dash = 0, gap = 0) {
    this.ctx.beginPath();
...
// ellipse
ellipse(x, y, radiusX, radiusY, color = 'black', fill = false, rotation = 0, startAngle = 0, endAngle = Math.PI * 2,dash = 0, gap = 0) {
    this.ctx.beginPath();
...
polygon(points, color = 'black', filled = true,dash = 0, gap = 0) {
...
triangle(x1, y1, x2, y2, x3, y3, color = 'black', filled = true, lineWidth=2,dash = 0, gap = 0) {
    ...
    // Draw the triangle
    this.polygon([x1, y1, x2, y2, x3, y3, x1, y1], color, filled);
...
ray(x0, y0, x1, y1, lineColor = 'black',lineWidth = 2, arrowWidth=5, arrowHeight=8, startArrow = true, endArrow = true,dash = 0, gap = 0) {
    ...
    this.ctx.lineWidth = lineWidth; // Set the line width
    this.ctx.strokeStyle = lineColor; // Set the line color
...
text(text, x, y, color = 'black', font = '12px Arial') {
        this.ctx.fillStyle = color;
        this.ctx.font = font;
