...
import { API_URL } from '../cmn/config.js';

/////////////////////////////////////////////////
export default class Agent {

static async create(route,despData={}){
const response = await ajaxPost(`${API_URL}/${route}/create` , {data:despData} );
return response;
}

static async update(route,despData={}){
const response = await ajaxPost(`${API_URL}/${route}/update` , {data:despData} );
return response;
}

static  async read(route,despData={}){
const response = await ajaxPost(`${API_URL}/${route}/read` , {data:despData} );
return response;
}

static async readone(route,despData={}){
const response = await ajaxPost(`${API_URL}/${route}/readone` , {data:despData} );
return response;
}

static async del(route,despData={}){
const response = await ajaxPost(`${API_URL}/${route}/delete` , {data:despData} );
return response;
}
